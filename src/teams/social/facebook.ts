...
export const socialFbGetPageInsights: ToolDefinition = {
  ...
  inputSchema: z.object({
    metrics: z.array(z.string()).default(["page_impressions", "page_engaged_users", "page_fans"]),
...
export const socialFbGetPostAnalytics: ToolDefinition = {
  ...
  inputSchema: z.object({
    postId: z.string(),
    metrics: z.array(z.string()).default(["post_impressions", "post_engaged_users", "post_clicks"]),
  }),
  execute: async (input): Promise<ToolResponse> => {
    const taskId = uuidv4();
    try {
