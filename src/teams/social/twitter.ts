...
function twHeaders(): Record<string, string> {
  return { Authorization: `Bearer ${process.env.TWITTER_BEARER_TOKEN || ""}`, "Content-Type": "application/json" };
}
...
export const socialTwPostTweet: ToolDefinition = {
  ...
  execute: async (input): Promise<ToolResponse> => {
    const taskId = uuidv4();
    try {
      const data = await httpPost(`${TWITTER_API}/tweets`, { text: input.text }, twHeaders());
...
export const socialTwGetTimeline: ToolDefinition = {
  ...
  inputSchema: z.object({ userId: z.string(), maxResults: z.number().default(10) }),
  execute: async (input): Promise<ToolResponse> => {
    const taskId = uuidv4();
    try {
      const data = await httpGet(`${TWITTER_API}/users/${input.userId}/tweets`, { max_results: input.maxResults }, twHeaders());
      return makeSuccessResponse(taskId, { timeline: data });
    } catch (err) { return makeErrorResponse(taskId, String(err)); }
  },
};

export const socialTwitterTools: ToolDefinition[] = [socialTwPostTweet, socialTwGetTimeline];
