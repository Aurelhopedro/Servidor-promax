...
function liHeaders(): Record<string, string> {
  return { Authorization: `Bearer ${process.env.LINKEDIN_ACCESS_TOKEN || ""}`, "Content-Type": "application/json", "X-Restli-Protocol-Version": "2.0.0" };
}

export const socialLiPostText: ToolDefinition = {
  name: "social_li_post_text",
  description: "Publicar post de texto no LinkedIn",
  inputSchema: z.object({ authorUrn: z.string(), text: z.string() }),
  execute: async (input): Promise<ToolResponse> => {
    const taskId = uuidv4();
    try {
      const data = await httpPost(`${LINKEDIN_API}/ugcPosts`, { author: input.authorUrn, lifecycleState: "PUBLISHED", specificContent: { "com.linkedin.ugc.ShareContent": { shareCommentary: { text: input.text }, shareMediaCategory: "NONE" } }, visibility: { "com.linkedin.ugc.MemberNetworkVisibility": "PUBLIC" } }, liHeaders());
...
export const socialLinkedinTools: ToolDefinition[] = [socialLiPostText, socialLiGetProfile];
